...
for imgNum, image in enumerate(images): # For each image in list...
    ...
    #Ray: upper codes just remap and transform colors to currency
...
#this script transforms image to colors :
'''
if color_num <= 256, then palette records color index -> calculated rgb value
and the output pixels are the transformed indices of the color
to transform back, need to map the palette
and if color_num > 256, no need to transform and just allocate size * 3 pixel matrix(r,g,b)
'''